...
//     let query = diesel::sql_query("SELECT id,salary,address,age,name FROM test.company");

//     let recipients = query.get_result::<Company>(conn);

//     // let data = diesel::sql_query("SELECT 1 AS foo, 2 AS bar").get_result(&conn);
//     // let x = query.get_results::<(i32,f32,String,i32,String)>(&conn);
...
//     HttpResponse::Ok().content_type("plain/text")
//         .header("X-Hdr", "sample")
//         .body("data")
